...
#endif
	int
	set_mib(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	case INT_T:
		...
		if (strcmp(entry->name, "wsc_enable") == 0)
		{
			...
			else if ((priv->pmib->wscEntry.wsc_enable == 0) && (int_val == 1))
			{
				...
				/*fixed for IOT issue */
				if ((OPMODE & (WIFI_STATION_STATE | WIFI_AUTH_SUCCESS | WIFI_ASOC_STATE)) == (WIFI_STATION_STATE | WIFI_AUTH_SUCCESS | WIFI_ASOC_STATE))
				{
...
/* WPS2DOTX   */
	...
	case WSC_IND_T:
		...
		else if (strcmp(entry->name, "wps_led_control") == 0)
		{ /*event to upnp*/
		...
		else if (strcmp(entry->name, "wps_debug") == 0)
		{ /*event to wps*/
			priv->pshare->WSC_CONT_S.debug = _atoi(arg_val, 10);
		}
		else if (strcmp(entry->name, "wps_reinit") == 0)
		{ /*event to wps*/
...
#endif
	int
	get_mib(struct rtl8192cd_priv *priv, unsigned char *data)
{
#ifdef __OSK__
#undef PRINT_INFO
...
#define POWER_RANGE_CHECK(val) (((val) > 0xff) ? 0xff : ((val < 0) ? 0 : val))
							   ...
							   char *CCKTxAgc_A, char *CCKTxAgc_B)
{
	...
	if ((priv->pshare->rf_ft_var.txPowerPlus_cck_11 != 0x7f) || (priv->pshare->rf_ft_var.txPowerPlus_cck_5 != 0x7f) || (priv->pshare->rf_ft_var.txPowerPlus_cck_2 != 0x7f) || (priv->pshare->rf_ft_var.txPowerPlus_cck_1 != 0x7f))
	{
	...
	if ((priv->pshare->rf_ft_var.txPowerPlus_ofdm_18 != 0x7f) || (priv->pshare->rf_ft_var.txPowerPlus_ofdm_12 != 0x7f) || (priv->pshare->rf_ft_var.txPowerPlus_ofdm_9 != 0x7f) || (priv->pshare->rf_ft_var.txPowerPlus_ofdm_6 != 0x7f) || (priv->pshare->rf_ft_var.txPowerPlus_ofdm_54 != 0x7f) || (priv->pshare->rf_ft_var.txPowerPlus_ofdm_48 != 0x7f) || (priv->pshare->rf_ft_var.txPowerPlus_ofdm_36 != 0x7f) || (priv->pshare->rf_ft_var.txPowerPlus_ofdm_24 != 0x7f))
	{
...
static void txpwr_dump(struct rtl8192cd_priv *priv, int start, int end)
{
	...
	else if (end <= 14)
	{
		for (channel = start; channel <= end; channel++) //_TXPWR_REDEFINE ?? DO NOT PRINT TOO MUCH
		{
			...
			if ((txpwr_lmt_CCK == 0) && (txpwr_lmt_OFDM == 0) && (txpwr_lmt_HT1S == 0) && (txpwr_lmt_HT1S == 0))
			{
...
static int iwpriv_atoi(struct rtl8192cd_priv *priv, unsigned char *data, unsigned char *buf, int len)
{
	...
	else if ((len - 1) == MACADDRLEN * 2)
	{ // user send 12 byte mac string
		SAVE_INT_AND_CLI(flags);
		SMP_LOCK(flags);
...
#endif
{
	...
	//	WAPI
	if (priv->pmib->wapiInfo.wapiType != wapiDisable || priv->pmib->dot1180211AuthEntry.dot11PrivacyAlgrthm == _WAPI_SMS4_)
	{
...
subtract all-rate power:\n	iwpriv wlanx set_power sub,{a,b,c,d},{power index}\n");
	...
	else if (ptxagc == priv->pshare->phw->CurrentTxAgcOFDM)
	{
		...
		case 9:
			...
			break;
		case 12:
			rate = TX_AGC_OFDM_12M;
			break;
		case 18:
			rate = TX_AGC_OFDM_18M;
			break;
		case 24:
			rate = TX_AGC_OFDM_24M;
			break;
		case 36:
			rate = TX_AGC_OFDM_36M;
			break;
		case 48:
			rate = TX_AGC_OFDM_48M;
			break;
		case 54:
			rate = TX_AGC_OFDM_54M;
			break;
		}
	}
	else if (ptxagc == priv->pshare->phw->CurrentTxAgcMCS)
...
#endif
	int
	rtl8192cd_ss_req(struct rtl8192cd_priv *priv, unsigned char *data, int len)
{
	INT8 ret = 0;
	unsigned long flags = 0;
	...
	if (!ret) // now, let's start site survey
	{
		...
		if (len == 2)
		{
			...
			{
				if (get_ss_level(priv) == 0)
				{							// exist some STA connected to root or VAP
...
#ifdef WIFI_SIMPLE_CONFIG
	else if (data_flags == 2)
	{ // get simple-config scan result, append WSC IE
		ret = priv->site_survey->count_backup;
		if (ioctl_copy_to_user((void *)data, (void *)&ret, 1))
		{
			RESTORE_INT(flags);
...
#endif
	void
	AddDelMCASTGroup2STA(struct rtl8192cd_priv *priv, unsigned char *mac2addr, int add)
{
	...
	if (add)
	{
		...
		if (pProxyEntry)
		{
			...
			if (pPathEntry)
			{
				...
				if (j == MAX_IP_MC_ENTRY && free >= 0)
				{ /*not found, and has free entry*/
...
void reset_default_sigma(struct rtl8192cd_priv *priv)
{
	...
	priv->pshare->rf_ft_var.gidforce = 0xff memset(priv->pshare->rf_ft_var.ndpa_mac, 0, 6);
#else
	priv->pshare->rf_ft_var.txforce = 0xff;
#endif
